...
import sys
import os
from PyQt6.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QWidget, QPushButton, QLabel, QSlider, QComboBox, QColorDialog, QGroupBox, QGridLayout, QFrame, QMessageBox, QSizePolicy
...
class ROGAuraGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.create_effects_section(main_layout)
    ...
    def create_effects_section(self, parent_layout):
        ...
        colors_group = QGroupBox("Quick Colors")
        ...
        single_label = QLabel("Single Zone Effects:")
        single_label.setObjectName("sectionLabel")
        effects_layout.addWidget(single_label, 0, 0, 1, 4)
        
        self.single_static_btn = QPushButton("Static Color")
        self.single_breathing_btn = QPushButton("Breathing")
        self.single_cycle_btn = QPushButton("Color Cycle")
        
        effects_layout.addWidget(self.single_static_btn, 1, 0)
        effects_layout.addWidget(self.single_breathing_btn, 1, 1)
        effects_layout.addWidget(self.single_cycle_btn, 1, 2)
        
        multi_label = QLabel("Multi Zone Effects:")
        multi_label.setObjectName("sectionLabel")
        effects_layout.addWidget(multi_label, 2, 0, 1, 4)
        
        self.multi_static_btn = QPushButton("Multi Static")
        ...
        effects_layout.addWidget(self.multi_static_btn, 3, 0)
        effects_layout.addWidget(self.multi_breathing_btn, 3, 1)
        effects_layout.addWidget(self.rainbow_btn, 3, 2)
        effects_layout.addWidget(self.rainbow_cycle_btn, 3, 3)
        
        params_label = QLabel("Parameters:")
        params_label.setObjectName("sectionLabel")
        effects_layout.addWidget(params_label, 4, 0, 1, 4)
        ...
        effects_layout.addWidget(speed_label, 5, 0)
        effects_layout.addWidget(self.speed_combo, 5, 1)
        
        self.color_picker_btn = QPushButton("Custom Color Picker")
        self.color_picker_btn.clicked.connect(self.open_color_picker)
        effects_layout.addWidget(self.color_picker_btn, 5, 2)
        ...
        effects_layout.addWidget(self.init_btn, 5, 3)
        
        self.single_static_btn.clicked.connect(lambda: self.apply_effect_with_color("single_static"))
        self.single_breathing_btn.clicked.connect(lambda: self.apply_effect_with_speed("single_breathing"))
        self.single_cycle_btn.clicked.connect(lambda: self.apply_effect_with_speed("single_colorcycle"))
        self.multi_static_btn.clicked.connect(lambda: self.apply_multi_effect("multi_static"))
    ...
    def create_color_buttons(self, layout):
        ...
        self.color_buttons = {}
        self.current_custom_color = "ffffff"
        ...
        for name, hex_color, cmd in colors:
            ...
            """)
            btn.clicked.connect(lambda checked, command=cmd: self.apply_color_command(command))
    ...
    def apply_effect_with_color(self, effect_name):
        success = self.backend.apply_single_effect(effect_name, self.current_custom_color)
        if success:
            self.status_bar.show_message(f"Applied {effect_name} with custom color")
        else:
    ...
    def apply_color_command(self, command):
        success = self.backend.apply_color(command)
        if success:
            self.status_bar.show_message(f"Applied color: {command.title()}")
        else:
